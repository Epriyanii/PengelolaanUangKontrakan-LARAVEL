...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <table class="table table-bordered">
    ...
    @forelse ($kontrakans as $kontrakan)
      <tr>
        <td>{{ $kontrakan->id }}</td>
        <td>{{ $kontrakan->no_kn }}</td>
        <td>{{ $kontrakan->status_kn }}</td>
        <td>{{ $kontrakan->harga_kn }}</td>
        <td>{{ $kontrakan->id_penghuni }}</td>
        <td class="text-center">
          <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('kontrakans.destroy', $kontrakan->id) }}" method="POST">
            <a href="{{ route('kontrakans.edit', $kontrakan->id) }}" class="btn btn-sm btn-primary">EDIT</a>
            @csrf
            @method('DELETE')
  ...
  {{ $kontrakans->links()}}
  <a href="{{ route('kontrakans.create') }}" class="w3-button w3-dark-grey">Tambah data  <i class="fa fa-arrow-right"></i></a>
</div>
